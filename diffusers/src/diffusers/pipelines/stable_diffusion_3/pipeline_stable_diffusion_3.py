...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    def __call__(
        ...
        # Dispersive Loss
        hook_handle = None,
        remove_hook_handle_after : Optional[float] = None,
        latent_reinit_every_timestep: Optional[bool] = False,
    ):
        r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == remove_hook_handle_after:
                    if hook_handle is not None:
                        hook_handle = hook_handle.remove() # Return None
                    else :
                        raise ValueError("hook_handle is None, but remove_hook_handle_after is set. Please check the hook_handle argument.")
                
                if hook_handle is not None and latent_reinit_every_timestep:
                    target_latent = latents[0].unsqueeze(0)
                    latents = torch.cat([target_latent] * len(latents))
                    print(f"hook_handle is set. Using same latent for all images for inference step : {i}.")
