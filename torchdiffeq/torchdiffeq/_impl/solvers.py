...
class FixedGridODESolver(metaclass=abc.ABCMeta):
    ...
    def __init__(self, func, y0, step_size=None, grid_constructor=None, interp="linear", perturb=False, dispersive_kwargs=None, **unused_kwargs):
        self.atol = unused_kwargs.pop('atol')
        self.hook_handle_list = None
        self.using_steps = None
        if dispersive_kwargs is not None:
            self.hook_handle_list = dispersive_kwargs.pop('hook_handle_list', None)
            self.using_steps = dispersive_kwargs.pop('using_steps', None)
    ...
    def integrate(self, t):
        ...
        j = 1
        y0 = self.y0
        for t_idx, (t0, t1) in tqdm(enumerate(zip(time_grid[:-1], time_grid[1:])), desc="Integrating", total=len(time_grid)-1):

            if self.hook_handle_list is not None and t_idx == self.using_steps:
