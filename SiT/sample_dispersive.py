...
def hook_fn(layer_idx, save_features=False, save_path=None):
    call_count = 0  # 호출 횟수 추적용 변수

    if save_features and save_path is None:
        raise ValueError("save_path must be provided when save_features is True")
    ...
    def hook_fn(module, input, output):
        ...
        if save_features:
            np.save(os.path.join(save_path, f"features_before_timestep_{call_count}_layer_{layer_idx}.npy"), features.detach().clone().to('cpu').numpy())
        ...
        if save_features:
            ...
            np.save(os.path.join(save_path, f"features_after_timestep_{call_count}_layer_{layer_idx}.npy"), features.detach().clone().to('cpu').numpy())
...
def main(mode, args):
    ...
    ckpt_path = args.ckpt or "/media/dataset2/jiwon/representation/SiT/checkpoint/SiT-XL-2-256.pt"
    ...
    # grid.png가 있으면 종료
    if os.path.exists(os.path.join(save_path, "grid.png")):
        print(f"이미지 저장 경로에 grid.png가 이미 존재합니다. {save_path}에서 작업을 중단합니다.")
        return
    
    hook_handle_list = []
    for layer_idx in layers_to_hook:
        hook_handle = model.blocks[layer_idx].register_forward_hook(hook_fn(layer_idx=layer_idx, save_features=args.save_features, save_path=save_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--using_steps", type=int, default=27, help="특징을 추출할 디노이징 스텝")
    ...
    parser.add_argument("--layer", type=int, default=24, help="layer to hook (0-27 for SiT-XL/2)")
    parser.add_argument("--baseline", action="store_true", help="baseline mode, no dispersive sampling")
    parser.add_argument("--same_noise", action="store_true", help="use same noise for all samples")
    parser.add_argument("--same_prompt", action="store_true", help="use same prompt for all samples")
    parser.add_argument("--save_features", action="store_true", help="save features before and after the update")
