...
def main():
    ...
    parser.add_argument("--update_scale", type=float, default=0.5, help="update scale")
    ...
    layers_to_hook = []
    for added_layer in range(1, 24):
        layers_to_hook.append(added_layer)
        ...
        if args.exp_name == 'baseline':
        ...
        elif args.exp_name == 'baseline_cfg':
            results = pipe(
                prompt=prompts,
                num_images_per_prompt=1,
                num_inference_steps=args.num_inference_steps,
            ...
            for layer_idx in layers_to_hook:
                hook_handle = pipe.transformer.transformer_blocks[layer_idx].register_forward_hook(hook_fn)
        ...
        save_path = os.path.join(args.save_path, prompt_list[0].replace(" ", "_")[:20]) # 첫 번째 프롬프트로 저장 경로 설정
        save_path = os.path.join(save_path, args.exp_name)
